...
uint8_t format_capacities_data[FORMAT_CAPACITIES_DATA_LENGTH] =
{
    ...
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 24),   /* number of blocks (MSB) */
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 16),   /* number of blocks (MSB) */
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 8),    /* number of blocks (MSB) */
    (uint8_t)(ISRAM_BLOCK_NUM - 1),           /* number of blocks (MSB) */
    0x02,                                     /* bit0-bit1:descriptor code */
    (uint8_t)((ISRAM_BLOCK_SIZE) >> 16),      /* block length (MSB) */
    (uint8_t)((ISRAM_BLOCK_SIZE) >> 8),       /* block length (MSB) */
    (uint8_t)(ISRAM_BLOCK_SIZE)               /* block length (MSB) */
};

/* USB mass storage read capacities data */
uint8_t read_capacities_data[READ_CAPACITIES_DATA_LENGTH] = 
{
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 24),   /* last logical block address (MSB) */
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 16),   /* last logical block address (MSB) */
    (uint8_t)((ISRAM_BLOCK_NUM - 1) >> 8),    /* last logical block address (MSB) */
    (uint8_t)(ISRAM_BLOCK_NUM - 1),           /* last logical block address (MSB) */

    (uint8_t)((ISRAM_BLOCK_SIZE) >> 24),      /* block length in bytes (MSB) */
    (uint8_t)((ISRAM_BLOCK_SIZE) >> 16),      /* block length in bytes (MSB) */
    (uint8_t)((ISRAM_BLOCK_SIZE) >> 8),       /* block length in bytes (MSB) */
    (uint8_t)(ISRAM_BLOCK_SIZE)               /* block length in bytes (MSB) */
};
    ...
    switch (scsi_cmd[0]) {
        ...
        case READ_10:
            ...
            if (COMMAND_STAGE == g_bbb_transport_stage) {
                ...
                block_addr *= ISRAM_BLOCK_SIZE;
                block_len *= ISRAM_BLOCK_SIZE;

                len = USB_MIN(block_len, MSC_MEDIA_PACKET_SIZE);
                sram_multi_blocks_read (bbb_data, 
                                        block_addr, 
                                        ISRAM_BLOCK_SIZE, 
                                        len / ISRAM_BLOCK_SIZE);
                ...
                if (0 == block_len) {
                    g_bbb_transport_stage = STATUS_STAGE;
                    csw_tag = bbb_cbw.dCBWTag;
                    csw_data_residue = bbb_cbw.dCBWDataTransferLength - ((scsi_cmd[7] << 8) | scsi_cmd[8]) * ISRAM_BLOCK_SIZE;
                    csw_status = 0;
                    len = 0;
                    bbb_set_csw(csw_tag, csw_data_residue, csw_status);
        ...
        case WRITE_10:
            ...
            if (COMMAND_STAGE == g_bbb_transport_stage) {
                ...
                sram_multi_blocks_write (bbb_data, 
                                         block_addr, 
                                         ISRAM_BLOCK_SIZE, 
                                         len / ISRAM_BLOCK_SIZE);
