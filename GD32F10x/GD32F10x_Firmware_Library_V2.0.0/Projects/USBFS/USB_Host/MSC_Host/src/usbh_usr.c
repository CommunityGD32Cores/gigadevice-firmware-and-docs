...
void usbh_user_init(void)
{
    ...
    if (0U == startup) {
        ...
        lcd_log_print((uint8_t *)MSG_LIB_START, sizeof(MSG_LIB_START) - 1, WHITE);

        charform.char_color = RED;
...
void usbh_user_device_connected(void)
{
    ...
    lcd_log_print((uint8_t *)MSG_DEV_ATTACHED, sizeof(MSG_DEV_ATTACHED) - 1, WHITE);
}
...
{
    lcd_log_print((uint8_t *)MSG_UNREC_ERROR, sizeof(MSG_UNREC_ERROR) - 1, WHITE);
}
...
void usbh_user_device_disconnected (void)
{
    ...
    lcd_log_print((uint8_t *)MSG_DEV_DISCONNECTED, sizeof(MSG_DEV_DISCONNECTED) - 1, WHITE);
...
{
    lcd_log_print((uint8_t *)MSG_RESET_DEV, sizeof(MSG_RESET_DEV) - 1, WHITE);
}
...
void usbh_user_device_speed_detected(uint8_t device_speed)
{
    if (HPRT_PRTSPD_HIGH_SPEED == device_speed) {
        lcd_log_print((uint8_t *)MSG_DEV_HIGHSPEED, sizeof(MSG_DEV_HIGHSPEED) - 1, WHITE);
    } else if(HPRT_PRTSPD_FULL_SPEED == device_speed) {
        lcd_log_print((uint8_t *)MSG_DEV_FULLSPEED, sizeof(MSG_DEV_FULLSPEED) - 1, WHITE);
    } else if(HPRT_PRTSPD_LOW_SPEED == device_speed) {
        lcd_log_print((uint8_t *)MSG_DEV_LOWSPEED, sizeof(MSG_DEV_LOWSPEED) - 1, WHITE);
    } else {
        lcd_log_print((uint8_t *)MSG_DEV_ERROR, sizeof(MSG_DEV_ERROR) - 1, WHITE);
    }
}
...
void usbh_user_device_desc_available(void *device_desc)
{
    ...
    lcd_log_print((uint8_t *)TempStr, str_len, WHITE);
                                          ...
                                          usb_descriptor_endpoint_struct *ep_desc)
{
    ...
    if (0x08U  == (*id).bInterfaceClass) {
        lcd_log_print((uint8_t *)MSG_MSC_CLASS, sizeof(MSG_MSC_CLASS) - 1, WHITE);
    } else if (0x03U  == (*id).bInterfaceClass) {
        lcd_log_print((uint8_t *)MSG_HID_CLASS, sizeof(MSG_HID_CLASS) - 1, WHITE);
    }
}
...
void usbh_user_enumeration_finish(void)
{
    ...
    lcd_log_print((uint8_t *)MSG_DEV_ENUMERATED, sizeof(MSG_DEV_ENUMERATED) - 1, WHITE);
    lcd_log_print((uint8_t *)Str1, sizeof(Str1) - 1, GREEN);
    lcd_log_print((uint8_t *)Str2, sizeof(Str2) - 1, GREEN);
}
...
{
    lcd_log_print((uint8_t *)MSG_DEV_NOSUP, sizeof(MSG_DEV_NOSUP) - 1, WHITE);
}
...
{
    lcd_log_print((uint8_t *)MSG_OVERCURRENT, sizeof(MSG_OVERCURRENT) - 1, WHITE);
}
    ...
    switch(usbh_usr_application_state)
    {
        case USBH_USR_FS_INIT:
            ...
            if (FR_OK != f_mount(0, &fatfs)) {
                lcd_log_print((uint8_t *)MSG_FILE_NOTINIT, sizeof(MSG_FILE_NOTINIT) - 1, WHITE);

                return(-1);
            }

            lcd_log_print((uint8_t *)MSG_FILE_INIT, sizeof(MSG_FILE_INIT) - 1, WHITE);
            ...
            if (DISK_WRITE_PROTECTED == usbh_msc_param.msc_write_protect) {
                lcd_log_print((uint8_t *)MSG_WR_PROTECT, sizeof(MSG_WR_PROTECT) - 1, WHITE);
            }
        ...
        case USBH_USR_FS_READLIST:
            lcd_log_print((uint8_t *)MSG_ROOT_CONT, sizeof(MSG_ROOT_CONT) - 1, WHITE);
            lcd_log_print((uint8_t *)Str1, sizeof(Str1) - 1, GREEN);
            lcd_log_print((uint8_t *)Str4, sizeof(Str4) - 1, GREEN);
        ...
        case USBH_USR_FS_WRITEFILE:
            ...
            lcd_log_print((uint8_t *)MSG_Write_File, sizeof(MSG_Write_File) - 1, WHITE);

            if (DISK_WRITE_PROTECTED == usbh_msc_param.msc_write_protect) {
                lcd_log_print((uint8_t *)MSG_Write_Protect, sizeof(MSG_Write_Protect) - 1, WHITE);
            ...
            if (FR_OK == f_open(&file, "0:GD32.TXT", FA_CREATE_ALWAYS | FA_WRITE)) {
                ...
                if ((0U == bytesWritten) || (FR_OK != res)) {
                    lcd_log_print((uint8_t *)MSG_NOT_WRITE, sizeof(MSG_NOT_WRITE) - 1, WHITE);
                } else {
                    lcd_log_print((uint8_t *)MSG_CREATE_FILE, sizeof(MSG_CREATE_FILE) - 1, WHITE);
                }
            ...
            usbh_usr_application_state = USBH_USR_FS_DEMOEND;
            lcd_log_print((uint8_t *)Str3, sizeof(Str3) - 1, GREEN);
            break;
...
static uint8_t explore_disk (char* path, uint8_t recu_level)
{
    ...
    if (res == FR_OK) {
        while (hcd_is_device_connected(&usb_core_dev)) {
            ...
            if (1U == recu_level) {
                uint8_t temp[] = "   |__";
                lcd_log_print((uint8_t *)temp, strlen((const char *)temp), WHITE);
            ...
            if (AM_DIR == (fno.fattrib & AM_MASK)) {
                lcd_log_print((uint8_t *)fno.fname, strlen(fno.fname), RED);
            } else {
                lcd_log_print((uint8_t *)fno.fname, strlen(fno.fname), WHITE);
            }
