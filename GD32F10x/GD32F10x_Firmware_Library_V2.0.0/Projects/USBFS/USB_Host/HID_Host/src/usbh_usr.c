...
{
    lcd_log_print((uint8_t *)USB_HID_MouseStatus, sizeof(USB_HID_MouseStatus) - 1, WHITE);
...
void  usr_keybrd_init (void)
{
    ...
    lcd_log_print((uint8_t *)USB_HID_KeybrdStatus, sizeof(USB_HID_KeybrdStatus) - 1, WHITE);

    lcd_log_print((uint8_t *)MSG_USE_KYBD, sizeof(MSG_USE_KYBD) - 1, WHITE);
...
void usr_keybrd_process_data (uint8_t data)
{
    ...
    if (keybrd_char_xpos <= KYBRD_LAST_LINE) {
        lcd_rectangle_fill(32, 0, 144, 320, BLACK);
