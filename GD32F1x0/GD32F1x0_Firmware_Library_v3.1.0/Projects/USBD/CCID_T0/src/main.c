...
usbd_core_handle_struct  usb_device_dev = 
{
    ...
    .class_init = ccid_init,
    .class_deinit = ccid_deinit,
    .class_req_handler = ccid_req_handler,
    .class_data_handler = ccid_data_handler
};
...
int main(void)
{
    ...
    while (1)
    {
        if (usb_device_dev.status == USBD_CONFIGURED)
        {
            ...
            CCID_IntMessage(&usb_device_dev);  
        }
    }
}
