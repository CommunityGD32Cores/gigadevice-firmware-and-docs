...
/**
  * @brief  Provides the ICC parameters to the host 
...
/**
  * @brief  Set the ICC parameters to the default 
  *         Response to this command message is the RDR_to_PC_Parameters
  * @param  None 
  * @retval status of the command execution 
  */
...
uint8_t  PC_to_RDR_SetParameters(void)
{
    ...
    if ((Ccid_bulkout_data.dwLength == 5) && (Ccid_bulkout_data.bSpecific_0 != 0))
    {
        error = SLOTERROR_BAD_PROTOCOLNUM;
    }

    /* for Protocol T=1 (bProtocolNum=1) (dwLength=00000007h) */
    if ((Ccid_bulkout_data.dwLength == 7) && (Ccid_bulkout_data.bSpecific_0 != 1))
    {
...
/**
  * @brief  Execute the Abort command from host, This stops all Bulk transfers 
  *         from host and ICC
  *         Response to this command message is the RDR_to_PC_SlotStatus
  * @param  None 
  * @retval status of the command execution 
...
/**
  * @brief  Execute the PC_TO_RDR_T0APDU command from host
  *         Response to this command message is the RDR_to_PC_SlotStatus
  * @param  None 
  * @retval status of the command execution 
  */
...
/**
  * @brief  Execute the PC_TO_RDR_MECHANICAL command from host
  *         Response to this command message is the RDR_to_PC_SlotStatus
  * @param  None 
  * @retval status of the command execution 
  */
...
/**
  * @brief  Set the required Card Frequency and Data rate from the host. 
...
/**
  * @brief  Execute the Secure Command from the host. 
  *         Response to this command message is the RDR_to_PC_DataBlock
  * @param  None 
  * @retval status of the command execution 
  */
...
/**
  * @brief  Provide the data block response to the host 
...
/**
  * @brief  Provide the Slot status response to the host 
...
void RDR_to_PC_SlotStatus(uint8_t  errorCode)
{
    ...
    Transfer_Data_Request((uint8_t*) (&Ccid_bulkin_data), LEN_RDR_TO_PC_SLOTSTATUS);
}
...
static uint8_t CCID_CheckCommandParams (uint32_t param_type)
{
    ...
    if (parameter & CHK_ACTIVE_STATE)
    {
        ...
        if (SC_GetState() != SC_ACTIVE_ON_T1)
#endif
        {
