...
esint8 ioman_releaseSector(IOManager *ioman,euint8* buf)
{
	...
	bp=ioman_getBp(ioman,buf);
        ...
        if(bp == 0xffff)
        { /* Error Condition Check for the Disconnection event.
             Pls note that ioman_getBp function puts (-1) in the variable for 
             the Error case, this is not handled properly causing the HardFaut 
             in the micro-controller. 
          We shall return some other value as returning 0 is for GOOD !!!
          But the upper layers will handle the Disconnection Event
          */
          
          return 0;
        }
     /* Error Case handling ends, Need to be updated by EFSL discussion forum */
