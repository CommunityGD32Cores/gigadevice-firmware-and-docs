...
enum tfm_plat_err_t tfm_spm_hal_init_isolation_hw(void)
{
    ...
    /* configure non-secure memory spaces and flash watermarks for NSPE */
    ret = flash_secure_config();
    if (ret != ARM_DRIVER_OK) {
        return TFM_PLAT_ERR_SYSTEM_ERR;
    }
    sram_mpcbb_config();
    gpio_nonsecure_config();
...
const struct mpu_armv8m_region_cfg_t region_cfg[] = {
           ...
           /* NSPM PSP */
           {
               MPU_REGION_NS_STACK,
#if defined (__ICCARM__)
               (uint32_t)&ARM_LIB_STACK$$Base,
               (uint32_t)&ARM_LIB_STACK$$Limit,
#else
