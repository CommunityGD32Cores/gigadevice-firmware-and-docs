...
#error TFM_LVL is not defined!
    ...
    extern uint32_t TFM_SECURE_STACK$$Base;
    extern struct iovec_args_t TFM_SECURE_STACK$$Limit[];
#else
...
#if defined (__ICCARM__)
    return &(TFM_SECURE_STACK$$Limit[-1]);
#else
                                           ...
                                           uint32_t excReturn)
{
     ...
     */
#if defined (__ICCARM__)
    partition_psp = (uint32_t)&(TFM_SECURE_STACK$$Limit[-1]);
    partition_psplim = (uint32_t)&TFM_SECURE_STACK$$Base;
#else
    partition_psp =
...
static enum tfm_status_e tfm_return_from_partition(uint32_t *excReturn)
{
    ...
    if (!(return_partition_flags & SPM_PART_FLAG_APP_ROT) ||
        ...
        *excReturn = ret_part_data->lr;
        ...
        uint32_t psp_stack_bottom = ARM_LIB_STACK$$Base;
        tfm_arch_set_psplim(psp_stack_bottom);
        iovec_args = &(TFM_SECURE_STACK$$Limit[-1]);
#else
