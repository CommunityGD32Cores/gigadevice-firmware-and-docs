...
int bind_mqtt_init(void *handle, void *mqtt_handle)
{
    ...
    ret = IOT_MQTT_Subscribe(bind_handle->mqtt_handle, buff, (iotx_mqtt_qos_t)1, bind_mqtt_deal_match_repy, bind_handle);
    if (ret < 0) {
        return ret;
    }
    memset(buff, 0, BIND_TOPIC_BUFFER_LEN);
    bind_get_topic(BIND_ENUM_TOPIC_RESET_REPLY, buff, BIND_TOPIC_BUFFER_LEN);
    ret = IOT_MQTT_Subscribe(bind_handle->mqtt_handle, buff, (iotx_mqtt_qos_t)1, bind_mqtt_deal_reset_repy, bind_handle);
    if (ret < 0) {
        return ret;
    }
    ...
    ret = IOT_MQTT_Subscribe(bind_handle->mqtt_handle, buff, (iotx_mqtt_qos_t)1, bind_mqtt_deal_event, bind_handle);
...
static void bind_mqtt_deal_event(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    switch (msg->event_type) {
        case IOTX_MQTT_EVENT_PUBLISH_RECEIVED: {
            ...
            callback = (bind_event_callback)iotx_event_callback(ITE_BIND_EVENT);
            if (callback) {
                callback(params_buf);
            }
