...
uint32 ParsePacket( ppacket pRxBuf )
{
    ...
    else if( ((pRxBuf->type)&(LOCAL_DATA_IN)) == LOCAL_DATA_IN )
    {
        ...
        GAgent_Printf( GAGENT_DEBUG,"LOCAL_DATA_IN cmd : %04X", *(pRxBuf->phead+4) );
    ...
    else if( ((pRxBuf->type)&(LAN_TCP_DATA_IN)) == LAN_TCP_DATA_IN )
    {
        ...
        cmd = ntohs(cmd);
        GAgent_Printf( GAGENT_DEBUG,"LAN_TCP_DATA_IN cmd : %04X", cmd);
...
int8 dealPacket( pgcontext pgc, ppacket pTxBuf )
{
    ...
    {
        GAgent_Printf( GAGENT_DEBUG,"dealPacket(): packet Type : LAN_TCP_DATA_OUT ");
