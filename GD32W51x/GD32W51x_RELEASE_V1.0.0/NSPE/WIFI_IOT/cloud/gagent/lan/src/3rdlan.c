...
int32 GAgent_JD_Get_Feedid_Key( GAgent3Cloud  *gagent3rdcloud,int8 *buf )
{
    ...
    if( 0!=memcmp( gagent3rdcloud->u_info.jdinfo.access_key,p_start,(p_end-p_start) ) )
    {
        isChange=1;
        memcpy( gagent3rdcloud->u_info.jdinfo.access_key,p_start,(p_end-p_start) );
        gagent3rdcloud->u_info.jdinfo.access_key[p_end-p_start]='\0';
        GAgent_Printf( GAGENT_DEBUG,"Got a new accesskey:%s",gagent3rdcloud->u_info.jdinfo.access_key );
    }
    ...
    if( 0!=memcmp(gagent3rdcloud->u_info.jdinfo.feed_id,p_start,(p_end-p_start)) )
    {
        isChange=1;
        memcpy(gagent3rdcloud->u_info.jdinfo.feed_id,p_start,(p_end-p_start));
        gagent3rdcloud->u_info.jdinfo.feed_id[p_end-p_start]='\0';
        GAgent_Printf( GAGENT_DEBUG,"Got a new feedid:%s",gagent3rdcloud->u_info.jdinfo.feed_id );
    }
    if( 0==isChange )
        return 1;
    else
    {
        gagent3rdcloud->u_info.jdinfo.tobeuploaded = 1;
        gagent3rdcloud->u_info.jdinfo.ischanged = 1;
        return 0;
    }
}
    ...
    switch( enctype )
        ...
        case APP2WIFI_BROADCAST:
            ...
            GAgent_JD_Discover_Ack( pgc->ls.udp3rdCloudFd,paddr,pgc->minfo.szmac,
                                    pgc->gc.cloud3info.u_info.jdinfo.product_uuid,
                                    pgc->gc.cloud3info.u_info.jdinfo.feed_id);
            break;
        case APP2WIFI_WRITE:
            ...
            if( 0==ret )
            {
                ...
                GAgent_Printf( GAGENT_INFO,"new accesskey:%s len=%d",
                                pgc->gc.cloud3info.u_info.jdinfo.access_key,
                                strlen(pgc->gc.cloud3info.u_info.jdinfo.access_key));
                GAgent_Printf( GAGENT_INFO,"new feedid:%s len=%d",
                                pgc->gc.cloud3info.u_info.jdinfo.feed_id,
                                strlen(pgc->gc.cloud3info.u_info.jdinfo.feed_id));
            }
            else if( 1==ret )
            {
            ...
            if( 1==pgc->gc.cloud3info.u_info.jdinfo.tobeuploaded )
            {
