...
void send_broadCastPacket(pgcontext pgc,uint8* ptxBuf,uint16 cmdWord)
{
    ...
    if((RET_SUCCESS == ret) && (INVALID_SOCKET != pgc->ls.udpBroadCastServerFd))
    {
        ...
        GAGENT_PRINT_DATA(GAGENT_DEBUG, "\n[UDP Broadcast TX]", ptxBuf, len);
        ret = Socket_sendto(pgc->ls.udpBroadCastServerFd, ptxBuf, len, &pgc->ls.addr, sizeof(pgc->ls.addr));
        GAgent_Printf( GAGENT_DEBUG,"Send BroadCast fd=%d cmd=%04X ret :%d",pgc->ls.udpBroadCastServerFd, cmdWord,ret );
    }
...
static void Lan_Upload_AllApp(pgcontext pgc, ppacket pTxBuf)
{
    ...
    for(i = 0; i < LAN_TCPCLIENT_MAX; i++)
    {
        ...
        {
            GAGENT_PRINT_DATA(GAGENT_INFO, "\n[TCP TX BC]", pTxBuf->phead, (u32)(pTxBuf->pend - pTxBuf->phead));
...
void Lan_sendTcpData(pgcontext pgc, int32 fd, uint16 cmd, int32 sn, ppacket pTxBuf)
{
    ...
    *(uint16 *)(pTxBuf->phead + offset) = htons(cmd);
    ...
    if(fd >= 0)
    {
        GAGENT_PRINT_DATA(GAGENT_INFO, "\n[TCP TX]", pTxBuf->phead, (u32)(pTxBuf->pend - pTxBuf->phead));
...
*/
    ...
    if(INVALID_SOCKET == pgc->ls.udpBroadCastServerFd)
    {
       ...
       Lan_CreateUDPBroadCastServer(&(pgc->ls.udpBroadCastServerFd),LAN_UDP_BROADCAST_SERVER_PORT, &pgc->ls.addr);
    }
}
/****************************************************************
                ...
                if( 1==pgc->rtinfo.waninfo.Cloud3Flag )
                {
                    if( 0==strcmp(pgc->gc.cloud3info.cloud3Name,"jd" ) )
                    {
                        if( pgc->ls.udp3rdCloudFd <=0 )
                        {
                            ...
                            GAgent_Printf( GAGENT_DEBUG,"3rdCloud UUID:%s",pgc->gc.cloud3info.u_info.jdinfo.product_uuid );
