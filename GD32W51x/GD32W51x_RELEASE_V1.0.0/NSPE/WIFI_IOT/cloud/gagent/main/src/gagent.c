...
void GAgent_Init( pgcontext *pgc )
{
    GAgent_DevInit( *pgc );
    GAgent_NewVar( pgc );
    GAgent_loglevelSet(/*GAGENT_DUMP*/GAGENT_INFO);
...
void GAgent_VarInit( pgcontext *pgc )
{
    ...
    (*pgc)->rtinfo.waninfo.cloud_config_sema = xSemaphoreCreateCounting(300, 0);
    (*pgc)->rtinfo.waninfo.m2m_rxevent = 0;//initialization m2m receive event
    (*pgc)->rtinfo.waninfo.http_rxevent = 0;//initialization  http receive event
    ...
    if(((*pgc)->gc.magicNumber != GAGENT_MAGIC_NUM) && ((*pgc)->gc.magicNumber != GAGENT_MAGIC_NUM_CONFIG))
    {
...
int32 GAgent_SelectFd(pgcontext pgc,int32 sec,int32 usec )
{
    ...
    int32 select_fd=0;
#ifdef CONFIG_GAGENT_CLOUD_TASK
    int32 mqtt_fd = pgc->rtinfo.waninfo.m2m_socketid;
    int32 http_fd = pgc->rtinfo.waninfo.http_socketid;
#endif
...
#ifdef CONFIG_GAGENT_CLOUD_TASK
    pgc->rtinfo.waninfo.m2m_rxevent = (mqtt_fd>0) ? FD_ISSET( mqtt_fd, &pgc->rtinfo.readfd):0;
    pgc->rtinfo.waninfo.http_rxevent = (http_fd>0) ? FD_ISSET( http_fd, &pgc->rtinfo.readfd):0;
#endif
        if( ret==0 )
        {
            //Time out.
...
void GAgent_RefreshIPTick( pgcontext pgc,uint32 dTime_s )
{
    ...
    if( (pgc->rtinfo.waninfo.RefreshIPLastTime) >= (pgc->rtinfo.waninfo.RefreshIPTime) )
    {
        ...
        {
            GAgent_Printf( GAGENT_INFO," %s : %s",HTTP_SERVER,tmpip);
...
void GAgent_Config( uint8 typed,pgcontext pgc )
{
    switch( typed )
    {
        ...
        case 2:
        {
            ...
            GAgent_DevLED_Red( 0 );
            GAgent_Printf( GAGENT_INFO,"Open Hybrid Config...");
...
uint8 GAgent_ExitTest( pgcontext pgc )
{
    ...
    GAgent_SetWiFiStatus(pgc,WIFI_MODE_BINDING, 0);	 //close onbinding
    GAgent_DRVWiFi_StationDisconnect();
    GAgent_SetWiFiStatus( pgc,WIFI_MODE_TEST,0 );
    pgc->rtinfo.GAgentStatus &= LOCAL_WIFI_LEVEL_MASK;
    GAgent_DevEraseWiFiConfigData(&(pgc->gc));
