#ifndef _GAGENT_TYPEDEF_H_
#define _GAGENT_TYPEDEF_H_
...
typedef struct GAGENT_CONFIG
{
    ...
    /*add wifi more info, FY*/
    uint8 wifi_bssid[6];
    uint32 security_mode;
    uint32 config_method; //0
    /*add wifi more info end, FY*/
    int8 rsvd[246];
    /** 3rd cloud **/
...
typedef struct _waninfo
{
    ...
    int32 m2m_socketid;
    int32 wanclient_num;
#if 1//#ifdef CONFIG_GAGENT_CLOUD_TASK
    void *cloud_config_sema;
    uint16 http_rxevent; // 1: received http packet
    uint16 m2m_rxevent;  // 1: received m2m packet
#endif
