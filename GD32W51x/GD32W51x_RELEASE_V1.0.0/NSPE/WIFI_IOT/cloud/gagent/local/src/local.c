...
/*bind PF_SendData2MCU to uart_write */
...
int32 GAgent_LocalDataWriteP0( pgcontext pgc,int32 fd,ppacket pTxBuf,uint8 cmd )
{
    ...
    *( pTxBuf->pend )  = GAgent_SetCheckSum(pTxBuf->phead, (pTxBuf->pend)-(pTxBuf->phead) );
    ...
    /* step 2. send data */
	/*if control data,transfer to philips first*/
    //if( pTxBuf->phead[4] = cmd)
    if( pTxBuf->phead[4])
    	Local_SendData( fd, pTxBuf->phead,sendLen );

    GAGENT_PRINT_DATA(GAGENT_INFO, "\n[UART TX]", pTxBuf->phead, sendLen);
...
uint32 GAgent_LocalDataHandle( pgcontext pgc,ppacket Rxbuf,int32 RxLen /*,ppacket Txbuf*/ )
{
    ...
    if( RxLen>0 )
    {
        ...
        {
            case MCU_REPORT:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_REPORT");
                ...
                break;
            case MCU_CONFIG_WIFI:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_CONFIG_WIFI");
                Local_Ack2MCU( pgc->rtinfo.local.uart_fd,sn,cmd+1 );
                ...
                break;
            case MCU_RESET_WIFI:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_RESET_WIFI");
                Local_Ack2MCU( pgc->rtinfo.local.uart_fd,sn,cmd+1 );
#if	 0
                GAgent_Clean_Config(pgc);
                msleep(2000);
                ...
                break;
            case MCU_INFO_CMD_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_INFO_CMD_ACK");
                ...
                break;
            case MCU_CTRL_CMD_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_CTRL_CMD_ACK");
                if(RET_SUCCESS == Local_CheckAck(pgc, cmd, sn))
                {
                    /* out to app and cloud, for temp */
                    Rxbuf->type = SetPacketType( Rxbuf->type,LOCAL_DATA_IN,1 );
                    ParsePacket( Rxbuf );
                    //GAgent_Printf( GAGENT_INFO, "pgc->ls.srcAttrs.fd=%d", pgc->ls.srcAttrs.fd);
                    //GAGENT_PRINT_DATA("phoneClientId", pgc->rtinfo.waninfo.srcAttrs.phoneClientId, PHONECLIENTID+1);
                ...
                break;
            case WIFI_STATUS2MCU_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = WIFI_STATUS2MCU_ACK");
                /*  */
            case WIFI_PING2MCU_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = WIFI_PING2MCU_ACK");
                Local_CheckAck(pgc, cmd, sn);
                ret = 0 ;
                break;
            case WIFI_TEST:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = WIFI_TEST");
                Local_Ack2MCU( pgc->rtinfo.local.uart_fd,sn,cmd+1 );
                GAgent_EnterTest( pgc );
                ret = 0;
                break;
            case MCU_ENABLE_BIND:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_ENABLE_BIND");
                Local_Ack2MCU( pgc->rtinfo.local.uart_fd,sn,cmd+1 );
                ...
                break;
            case MCU_REQ_GSERVER_TIME:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_REQ_GSERVER_TIME");
                ...
                *(uint16 *)(Rxbuf->ppayload) = htons(tm.year);
                ...
                break;
            case MCU_NEED_UPGRADE_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_NEED_UPGRADE_ACK");
                Local_CheckAck(pgc, cmd, sn);
                ret = 0 ;
                break;
            case MCU_READY_RECV_FIRMWARE:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = MCU_READY_RECV_FIRMWARE");
                ...
                break;
            case GAGENT_SEND_UPGRADE_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = GAGENT_SEND_UPGRADE_ACK");
                Local_CheckAck(pgc, cmd, sn);
                ret = 0 ;
                break;
            case GAGENT_STOP_SEND_ACK:
                GAgent_Printf( GAGENT_INFO, "UART RX CMD = GAGENT_STOP_SEND_ACK");
                Local_CheckAck(pgc, cmd, sn);
                ret = 0 ;
                break;
            default:
...
void GAgent_Local_Handle( pgcontext pgc,ppacket Rxbuf,int32 length )
{
    ...
    do
    {
        ...
        if(localDataLen > 0)
        {
            GAGENT_PRINT_DATA(GAGENT_INFO, "\n[UART RX]", Rxbuf->phead, localDataLen);
