...
class PatchSet(object):
  ...
  def pathmodify(self, old ,new):
    if old[0] == '"' and old[-1] == '"' and new[0] == '"' and new[-1] == '"':
      old = old[1:-1]
      new = new[1:-1]
  ...
  def apply(self, strip=0, root=None):
    ...
    for i,p in enumerate(self.items):
      ...
      old, new = self.pathmodify(old, new)
      filename = self.findfile(old, new)

      if not filename:
        if len(p.hunks)==1 and p.hunks[0].startsrc==0 and p.hunks[0].linessrc==0:
          filename=new
          debug("patch creates new file %s " % filename)
          # create directory
          filenameDir=os.path.join(os.path.dirname(filename), b".")
